...
export default function PostCard({post}) {
  ...
  const [Liked, setLiked] = useState(post.isLiked(userContext.username));
  ...
  useEffect(() => {
    post.updateCurrencyPrice();
  }, [post.currencies[0].value]);
  ...
  const RenderIsUsernameSameAsPostedBy = () => {
    if (userContext.username === post.postedBy) return <Image style = {{height: 50, width: 50, borderRadius: 15, marginLeft: 12, marginRight: 12}} source = {{uri: userContext.profilePicture}}/>;
    return (
      <Pressable onPress = {() => navigation.navigate('view profile', {postedByUsername: post.postedBy})}>
        <Image style = {{height: 50, width: 50, borderRadius: 15, marginLeft: 12, marginRight: 12}} source = {{uri: post.profilePicture}}/>
      </Pressable>
    );
  };
    ...
    <View style={{backgroundColor: 'transparent', borderRadius: 10, margin: 10}}>
      ...
      <View style={{flexDirection: 'row', position: 'absolute', top: 12, left: 10, zIndex: 1}}>
        ...
        <View style={{marginLeft: 5}}>
          <Text style={{fontSize: 15, fontWeight: 'bold', color: 'white', elevation: 1}}>{post.title}</Text>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <Image style={{height: 20, width: 20, marginTop: 4, borderRadius: 20}} source={{uri: post.updatedCurrencyList(post.currencies)[0].value}}/>
            <Text style={{color: 'white', fontSize: 15, elevation: 1, marginLeft: 5, fontWeight: '500'}}>{post.updatedCurrencyList(post.currencies)[0].price} </Text>
            <Text style={{color: 'white', fontSize: 11, elevation: 1, fontWeight: '500'}}>(${Number(post.USD).toLocaleString('en-US')})</Text>
          </View>
        </View>
      </View>
      <FlatList
        ...
        renderItem={({item}) => (
          <Pressable onPress={() => navigation.navigate('post details', {id: post.id})}>
